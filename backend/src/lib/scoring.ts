export const DOMAIN_CONFIG: Record<
  string,
  { maxScore: number; questionCount: number; statusRanges: { label: string; min: number; max: number; color: string }[] }
> = {
  Social: {
    maxScore: 45,
    questionCount: 9,
...
export function disabilityPercentage(totalScore: number) {
  ...
  if (totalScore <= 88) return 40;
  if (totalScore <= 105) return 50;
  if (totalScore <= 123) return 60;
  if (totalScore <= 140) return 70;
  if (totalScore <= 158) return 80;
  if (totalScore <= 175) return 90;
  return 100;
}

export function getDomainStatus(domain: string, score: number) {
  const config = DOMAIN_CONFIG[domain];
  if (!config) return { label: 'Unknown', color: '#6B7180' };
